        ...
        try:
            ...
            try:
                if self.config.database.provider.lower() == 'falkordb':
                    ...
                    # For Neo4j (default), build our SanitizingNeo4jDriver explicitly
                    # and pass it via graph_driver= (rather than uri/user/password,
                    # which would make Graphiti construct a plain Neo4jDriver).
                    # SanitizingNeo4jDriver flattens nested-dict attribute values to
                    # JSON strings at the write boundary; without it, constrained-
                    # decoding endpoints occasionally emit a nested object for an
                    # entity/edge attribute (e.g. Project.status={description: ...})
                    # and Neo4j rejects the whole write with CypherTypeError(Map{...}),
                    # aborting ingestion. See kg/sanitizing_driver.py.
                    from kg.sanitizing_driver import SanitizingNeo4jDriver

                    neo4j_driver = SanitizingNeo4jDriver(
                        uri=db_config['uri'],
                        user=db_config['user'],
                        password=db_config['password'],
                    )
