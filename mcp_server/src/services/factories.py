        ...
        match provider:
            case 'openai':
                ...
                if use_generic_client:
                    # Non-OpenAI endpoint (e.g. dasuapi relay). We deliberately do
                    # NOT use graphiti-core's OpenAIGenericClient here: its
                    # json_schema mode omits "strict": true, and dasuapi hard-fails
                    # that — the migration probe showed a narrative episode returns
                    # HTTP 400 ("additionalProperties must be false") and a
                    # conversational one returns a bare array the generic client
                    # can't parse (TypeError). Our ChatCompletionsClient sends
                    # strict json_schema (which dasuapi's constrained decoding
                    # honors), re-wraps bare-array responses, and only falls back to
                    # json_object on a genuine 400/422 schema-param rejection.
                    # See kg/chat_completions_client.py.
                    from kg.chat_completions_client import ChatCompletionsClient

                    return ChatCompletionsClient(config=llm_config, max_tokens=config.max_tokens)
                else:
